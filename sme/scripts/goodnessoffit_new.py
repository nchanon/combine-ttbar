...
# python scripts/goodnessoffit_new.py n_bjets Comb cLXX,cLXY,cLXZ,cLYZ 10 -99
#ntoys=100: fit 100 toys + data
#ntoys!=100: fit ntoys (and not data). if seed=-99, do the merging
...
if ntoys==100 or doFit==False:
    ...
    if ntoys==100:
        MCname = 'higgsCombine.mctoys_'+observable+'_'+year+'_'+wilsonserie+'.GoodnessOfFit.mH120.123456.root'
    if ntoys!=100 and ntoys!=-1:
	MCname = 'higgsCombine.mctoys_'+observable+'_'+year+'_'+wilsonserie+'_ntoys'+str(ntoys)+'.GoodnessOfFit.mH120.all.root'
	cmd3 = 'hadd -f '+MCname+' higgsCombine.mctoys_'+observable+'_'+year+'_'+wilsonserie+'_ntoys'+str(ntoys)+'.GoodnessOfFit.mH120.*.root'
	os.system(cmd3)
