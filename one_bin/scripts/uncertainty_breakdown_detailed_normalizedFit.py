...
#def compareSignalStrengthLists(mulist):
    #value_central_allbins[i][k]
...
nuis_time_corr = nuis_lumi_time_corr+','+nuis_exp_time_corr #+','+nuis_mcstat
nuis_time_corr_withMCstat = nuis_lumi_time_corr+','+nuis_exp_time_corr+','+nuis_mcstat
...
for k in range(len(list_nuisnames)):
    ...
    if k==0:
	fCovar.append(TFile(fCovarPrefix+".snapshot_"+observable+"_"+year+"_"+NuisanceGroup+'_'+asimov+".root"))
    if k>0 and k<len(list_nuisnames)-1:
	#if list_nuisnames[k]!="hdamp":
	fCovar.append(TFile(fCovarPrefix+".freeze"+list_nuisnames[k]+"_"+observable+"_"+year+"_"+NuisanceGroup+'_'+asimov+".root"))
	#else:
	#fCovar.append(TFile(fCovarPrefix+".freeze"+list_nuisnames[k]+"_"+observable+"_"+year+"_"+NuisanceGroup+'_'+asimov+"_testToys.root"))
    if k==len(list_nuisnames)-1:
	fCovar.append(TFile(fCovarPrefix+".freezeall_"+observable+"_"+year+"_"+NuisanceGroup+"_"+asimov+".root"))
...
hCovDown = []
for k in range(len(list_nuisnames)):
    if ((asimov=="asimov" and (list_nuisnames[k]=="hdamp" or list_nuisnames[k]=="bkgd_norm")) or (asimov=="data" and list_nuisnames[k]=="time_flat")) and year=="Comb" and doToysCovariance==True:
	print "Open covariance_fromToys_"+observable+"_"+year+"_"+NuisanceGroup+"_"+asimov+"_freeze"+list_nuisnames[k]+".root"
	if asimov=="asimov":
    	    externalFile = TFile("covariance_fromToys_"+observable+"_"+year+"_"+NuisanceGroup+"_freeze"+list_nuisnames[k]+".root")
	if asimov=="data":
	    externalFile = TFile("covariance_fromToys_"+observable+"_"+year+"_"+NuisanceGroup+"_freeze"+list_nuisnames[k]+"_data.root")
...
#Set measured signal strength
#ntimebin = 24
ttbar_yield_pernuis = []
mu_avg_pernuis = []
for k in range(len(list_nuisnames)):
    ttbar_yield = []
    for i in range(24):  #To be revisited for data (where it will be needed to do it for each of the nuisance group since mu can change slightly)
	ttbar_yield.append(value_central_allbins[i][k])
	print ttbar_yield[-1]
    mu_avg = sum(ttbar_yield)/ntimebin
    print 'mu_avg='+str(mu_avg)
    ttbar_yield_pernuis.append(ttbar_yield)
    mu_avg_pernuis.append(mu_avg)

#Compute Jacobian (to be updated for each nuis group)
hJacobian_pernuis = []
#hJacobian = TH2F("jacobian","jacobian",24,0,24,24,0,24)
jacobian_val  = 0.
for k in range(len(list_nuisnames)):
    hJacobian_pernuis.append(TH2F("jacobian"+list_nuisnames[k],"jacobian"+list_nuisnames[k],24,0,24,24,0,24))
    for i in range(24):
	for j in range(24):
	    jacobian_val = -ttbar_yield_pernuis[k][i]/ntimebin
	    #print str(ttbar_yield_pernuis[0][i])+" "+str(ntimebin)+" "+str(jacobian_val)
	    if (i==j):
		jacobian_val = jacobian_val + mu_avg_pernuis[k]
	    jacobian_val = jacobian_val / (mu_avg_pernuis[k]*mu_avg_pernuis[k])
	    hJacobian_pernuis[-1].SetBinContent(1+i, 1+j, jacobian_val)
...
for k in range(len(list_nuisnames)):
    ...
    for i in range(24):
        for j in range(24):
            matrix_Jacobian[i][j] = hJacobian_pernuis[k].GetBinContent(1+i, 1+j)
            matrix_JacobianTranspose[i][j] = hJacobian_pernuis[k].GetBinContent(1+j, 1+i)
    ...
    mu_norm_down = []
    for i in range(24):
        mu_norm.append(ttbar_yield_pernuis[k][i]/mu_avg) #different for each nuis fit 
	if doSubtractCovar==True:
