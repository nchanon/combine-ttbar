...
#Data fit
cmd1 = 'combine -M GoodnessOfFit inputs/combine_'+observable+'_24bins_'+year+'_workspace.root '
cmd1 += param
cmd1 += ' --algo='+algo+' -n .data_'+observable+'_'+year+'_'+algo+'_'+muValue

#MC toys
cmd2 = 'combine -M GoodnessOfFit inputs/combine_'+observable+'_24bins_'+year+'_workspace.root '
cmd2 += param
if algo=='saturated':
    cmd2 += ' --algo=saturated --toysFreq -t '+str(ntoys)
else:
    cmd2 += ' --algo='+algo+' -t '+str(ntoys)
cmd2 += ' -n .mctoys_'+observable+'_'+year+'_'+algo+'_'+muValue 
...
fData = TFile('higgsCombine.data_'+observable+'_'+year+'_'+algo+'_'+muValue+'.GoodnessOfFit.mH120.root')
tData = fData.Get('limit')
tData.GetEvent(0)
val_data = tData.GetLeaf('limit').GetValue()
print str(val_data)

fMC = TFile('higgsCombine.mctoys_'+observable+'_'+year+'_'+algo+'_'+muValue+'.GoodnessOfFit.mH120.123456.root')
tMC = fMC.Get('limit')
val_MC_min=9999999
val_MC_max=0
val_MC = []
...
Canvas.SaveAs('impacts/'+year+'/goodnessOfFit_differential_'+observable+'_'+year+'_'+algo+'_'+muValue+'.pdf')
