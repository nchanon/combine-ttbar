...
parser.add_argument('nuis', help='Splitting by nuis')
...
for asimov in list_asimov:
    for year in list_years:
        ...
        cmd4 = 'combine -M MultiDimFit higgsCombine.snapshot_'+year+stimebin+'_'+asimov+'.MultiDimFit.mH120.root -n .freeze'+nuis+'_'+year+stimebin+'_'+asimov+' '
        cmd4 += asimov_param(asimov)
        cmd4 += ' --parameters '+rbin+' --setParameterRanges '+rbin+'='+r_range #+' --floatOtherPOIs=1 '
        cmd4 += ' --algo grid --points '+str(npoints*3)+' '
        cmd4 += ' --freezeParameters rgx{.*'+nuis+'.*} --snapshotName MultiDimFit'

        cmd5 = "python plot1DScan.py higgsCombine.nominal_"+year+stimebin+'_'+asimov+".MultiDimFit.mH120.root --others 'higgsCombine.freeze"+nuis+"_"+year+stimebin+"_"+asimov+".MultiDimFit.mH120.root:Freeze"+nuis+":4' 'higgsCombine.freezeall_"+year+stimebin+'_'+asimov+".MultiDimFit.mH120.root:FreezeAll:2' -o freeze_all_"+year+"_"+asimov+" "
        cmd5  += "--POI "+rbin
        cmd5 += " --main-label "
        if (asimov=='asimov'): cmd5 += 'Asimov'
	else: cmd5 += 'Data'
        cmd5 += " --breakdown "+nuis+",OtherSyst,Stat "
	cmd5 += " -o impacts/"+year+"/UncertaintyBreakdown_"+observable+"_"+year+stimebin+"_"+asimov+"_"+rbin+"_"+nuis
        cmd5 += " > impacts/"+year+"/uncertainty_breakdown_"+observable+"_"+year+stimebin+'_'+asimov+'_'+rbin+"_"+nuis+".log"
...
for asimov in list_asimov:
    for year in list_years:
        ...
        file = open('impacts/'+year+'/uncertainty_breakdown_'+observable+"_"+year+stimebin+'_'+asimov+'_'+rbin+'_'+nuis+'.log')
        ...
        for line in file:
            ...
            for word in line.split():
                ...
                if (i==20): unc_nuis_neg.append(float(word[:-1]))
                if (i==22): unc_nuis_pos.append(float(word[:-1]))
	...
	rate_nuis.append([mu,mu-unc_nuis_neg[-1], unc_nuis_pos[-1]-mu])
        rate_stat.append([mu,mu-unc_noSyst_neg[-1], unc_noSyst_pos[-1]-mu])

print rate_total
print rate_stat

print 'Write '+'impacts/'+year+'/Latex_UncertaintyBreakdown_'+observable+stimebin+'_'+nuis+'.txt'
sys.stdout = open('impacts/'+year+'/Latex_UncertaintyBreakdown_'+observable+stimebin+'_'+nuis+'.txt', 'w')
...
for asimov in list_asimov:
	...
	#text += str(round(bestval[i], 3))+'^{+' + str(round(unc_total_pos[i]-bestval[i], 3)) + '}_{-' + str(round(bestval[i]-unc_total_neg[i], 3))+ '} '
        ...
        syst_nuis_up = math.sqrt((unc_total_pos[i]-bestval[i])*(unc_total_pos[i]-bestval[i])-(unc_nuis_pos[i]-bestval[i])*(unc_nuis_pos[i]-bestval[i]))
        syst_nuis_down = math.sqrt((unc_total_neg[i]-bestval[i])*(unc_total_neg[i]-bestval[i])-(unc_nuis_neg[i]-bestval[i])*(unc_nuis_neg[i]-bestval[i]))

	syst_other_up = math.sqrt((unc_nuis_pos[i]-bestval[i])*(unc_nuis_pos[i]-bestval[i])-(unc_noSyst_pos[i]-bestval[i])*(unc_noSyst_pos[i]-bestval[i])) 
	syst_other_down = math.sqrt((unc_nuis_neg[i]-bestval[i])*(unc_nuis_neg[i]-bestval[i])-(unc_noSyst_neg[i]-bestval[i])*(unc_noSyst_neg[i]-bestval[i]))

        #text += '=  '+ str(round(bestval[i], 3))+'^{+' + str(round(syst_nuis_up, 3)) + '}_{-' + str(round(syst_nuis_down, 3))+ '}$('+nuis+')'
	#text += '$^{+' + str(round(syst_other_up, 3)) + '}_{-' + str(round(syst_other_down, 3))+ '}$(other syst)'
	#text += '$^{+' + str(round(unc_noSyst_pos[i]-bestval[i], 3)) + '}_{-' + str(round(bestval[i]-unc_noSyst_neg[i], 3))+ '}$(stat) \\\\'

	text = '\\mathrm{'+asimov+' '+year+stimebin+'}: \mu='
	text += str(round(bestval[i], precision))+'^{+' + str(round(unc_total_pos[i]-bestval[i], precision)) + '}_{-' + str(round(bestval[i]-unc_total_neg[i], precision))+ '} '
	text += '=  '+ str(round(bestval[i], precision))+'^{+' + str(round(syst_nuis_up, precision)) + '}_{-' + str(round(syst_nuis_down, precision))+ '}\\mathrm{('+nuis+')}'
	text += '^{+' + str(round(syst_other_up, precision)) + '}_{-' + str(round(syst_other_down, precision))+ '}\\mathrm{(other syst)}'
	text += '^{+' + str(round(unc_noSyst_pos[i]-bestval[i], precision)) + '}_{-' + str(round(bestval[i]-unc_noSyst_neg[i], precision))+ '}\\mathrm{(stat)} \\\\'
	print text
	i=i+1
...
###    rbin = pois[j]
###
###    file = open('impacts/'+year+'/uncertainty_breakdown_'+year+stimebin+'_'+asimov+'_'+rbin+'.log')
###
###    i=0
###    for line in file:
###        uncert = 0
...
###################################################################################
###
###resultname = './impacts/'+year+'/differential_time_breakdown_'+year+stimebin
###
